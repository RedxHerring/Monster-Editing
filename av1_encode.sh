...
cd $1
searchstring="Monster_" # note the space at the end
for vid in *.mp4; do
    rest=${vid#*$searchstring}
    epnum=$(cut -c 1-2<<< $rest)
    if [[ "$epnum" == "01" ]]; then
        params="-vf subtitles=../../Subs/Definitive-Subs/Ep01-04-opening.ass"
    else
        params=""
    fi
    ffmpeg -n -loglevel warning -init_hw_device qsv=hw -filter_hw_device hw -i $vid -map 0 -c:v av1_qsv -preset veryslow \
        -global_quality:v 20 -extbrc 0 -look_ahead_depth 36 -pix_fmt p010le $params -c:a:0  libopus -b:a 108000 -vbr:a on "${vid%.*}.mkv"
done
