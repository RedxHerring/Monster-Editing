...
for vid in $rip_path/*mpg; do
    ...
    if [  -f "$fullname_out" ] && [ $over_write = false ]; then
        ...
        ffmpeg -y -loglevel warning -i $vid -map v:0 -map a:0 -map a:1 -map s:0 -map s:1 -vf "crop=1440:1080" \
        -c:v libsvtav1 -crf 21 -preset 4 -pix_fmt yuv420p10le -svtav1-params film-grain=4 -c:a:0 libvorbis -q:a:0 7 -c:a:1 libopus -b:a:1 108000 -vbr:a on -c:s copy "$fullname_out"
